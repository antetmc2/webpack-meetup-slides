...
```notes
Today Web apps are getting smarter and bigger and more complex. Even thought we have a lot of options for web management, we have issues like many "stuff" for using, a lot of files, order of adding stylesheets and scripts into HTML, file optimization and more. We have a solution which can help - Webpack.
```
...
```notes
We will create a Node.JS application. Node.JS -> because of simplicity, environment which allows running JavaScript on backend. Create folder, open console, npm init -y (no answering questions, quickly creating and skipping questions)
```
...
```notes
What else do we need to do? First we need to call npm install or shorter npm i where we install necessary dependencies.
Here we need to install webpack package with everything necessary for Webpack and webpack-cli which contains all Webpack CLI commmands.
...
```notes
So, now when we have a JavaScript file, what does do we need? We need an HTML file to process and use this script. But we need to think about one more thing. If we add only HTML, we will have to always write some code, save the file, run Webpack again, click F5 in Browser and repeat this again. There is one more thing we could use for developing and that is development server or more precisely webpack-dev-server because we want better development, automatic reloading and serving files on some location. After we install the devDependency, all we have to do is define run scripts. It is recommended to use webpack-dev-server for development only, while for production you should use more permanent solution like Apache or something similar. We will also pass the mode and now the only thing left to do is define how to fetch HTML.
```
...
<CodeSurfer
  title="We can also modify webpack-dev-server :)"
